...
<script>
  ...
  export default {
    ...
    data() {
      return {
        crumbs: [
          ...
          {name: '烟草逸事', path: '/interaction/tobaccoanecdotes'},
          {name: '文章详情', path: ''}
        ],
        articleDetails: {},
        timer: null
    ...
    beforeRouteEnter(to, from, next) {
      next(vm => {
        if(from.path =='/interaction/tobaccoanecdotes'){
          vm.crumbs = [
            {name: '互动交流', path: '/interaction'},
            {name: '烟草逸事', path: '/interaction/tobaccoanecdotes'},
          ]
    ...
    created() {
      //  /interaction/forinvestigation  征集调查
      //  /interaction/tobaccoanecdotes  烟草逸事
      console.log(this.$route.query);
      let result = this.foo.toString().includes('/interaction/tobaccoanecdotes');
      let name = this.$route.query.articleType;
      if(result){
        // this.crumbs = [
        //   {name: '互动交流', path: '/interaction'},
        //   {name, path: '/interaction/tobaccoanecdotes'},
        //   {name: '文章详情', path: ''}
        // ]
      }
      ARTICLE_TYPE.forEach(item=>{
        if(item.label==name){
          this.firstLoad({id: this.$route.query.id,typeCode: 'yancaoyishi'})
        }
      })
    ...
    mounted() {
      this.timer = setInterval(() => {
        if (this.articleDetails.content.length) {
          ...
          //           http://www.tobacco.gov.cn/attachfiles/2020/01/15/8bed89a57f7ebe28f93f2ed8d1ff94c7ba6a559a.jpg
    ...
    methods: {
      ...
      nextPreArticle(id) {
        ...
        // this.$router.push({path: '/interaction/tobaccoanecdotes/tobaccoanecdotesdetails?id=', query: {articleType:'烟草逸事'}});
        this.$router.push({
          path: '/interaction/tobaccoanecdotes/tobaccoanecdotesdetails?id=' + id,
          // query: {articleType: this.$route.query.articleType}
          query: {articleType: '烟草逸事'}
        });
        this.reload()
      }
    }
